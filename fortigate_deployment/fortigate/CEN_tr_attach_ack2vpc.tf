variable "ack2_vswitch0_cidr_block" {
default = ""
}

variable "ack2_vswitch1_cidr_block" {
default = ""
}
...
resource "alicloud_vswitch" "ack2_vswitch0" {
  ...
  cidr_block = var.ack2_vswitch0_cidr_block
...
resource "alicloud_vswitch" "ack2_vswitch1" {
 count=var.securehub=="1" ? 1 :0 
  ...
  vpc_id = module.vpc2.vpc-id
  cidr_block = var.ack2_vswitch1_cidr_block
  zone_id = var.centr["zone_id_2"]
}

resource "alicloud_cen_transit_router_vpc_attachment" "atta_ack2" {
 count =var.securehub == "1" ? 1:0
 depends_on =[alicloud_cen_transit_router_vpc_attachment.atta_ack1,alicloud_cen_instance.default]
  ...
  zone_mappings {
   zone_id = var.centr["zone_id_1"]
    vswitch_id = alicloud_vswitch.ack2_vswitch0[count.index].id
  }
  zone_mappings {
    zone_id = var.centr["zone_id_2"] 
    vswitch_id = alicloud_vswitch.ack2_vswitch1[count.index].id
  }
...
resource "alicloud_cen_transit_router_route_table_propagation" "propagte_spoke_a_vpc_route_East-West_rt" {
  ...
  transit_router_attachment_id = alicloud_cen_transit_router_vpc_attachment.atta_ack2[count.index].transit_router_attachment_id
}

...
variable "instance_name" {
  type    = string
  default = "terraform-win"
  description = "WINDOWS2019的实例名字前缀"
}
...
//  default="ecs.ic5.large"
 description = "WINDOWS2019实例的实例类型，默认auto为按照CPU，MEMORY自动选择"
}
variable "password" {
 default=""
}
...
variable instance_type_allowed_eni_amount {
 default=3
  description = "WINDOWS2019实例的实例类型的网卡数量"
 }
variable cpu {
 default=4
  description = "WINDOWS2019实例的实例类型的CPU核心数"
 }
 
variable memory {
  default =8
    description = "WINDOWS2019实例的实例类型的内存数量，单位为G "
}
